            ...
            {[
              {
                ...
                project: "豆包 - AI生成PPT",
                period: "2025.09 - 至今",
                description: `豆包办公 AI PPT 评测体系构建与评测闭环
评测标准设计
1) 拆解 AI生成PPT 能力结构，构建 内容质量 / 视觉设计 / 排版稳定性 3大评估维度，围绕 指令遵循、逻辑一致性、视觉美观度等关键问题点设计评分规则，将主观评估逻辑转化为可执行的打分标准。
2) 在端到端评测流程中定义问题点识别框架与评分口径，提升人工评测结果的一致性与可解释性，并为后续自动化评测体系建设奠定基础。
测试集与Benchmark构建
3) 构建 分层模型评测测试集：包含 50组日常能力监控集 + 多个专项评测集，覆盖 内容生成、附件解析、模板复用、生成后编辑 等核心场景。
4) 设计 30–50题规模专项测试集（内容生成 / 附件解析 / 模板能力）及 100题深度评测集（复杂长文档解析、跨页逻辑一致性、排版冲突）。
5) 基于稳定评测集建立竞品Benchmark，对国内外 AI PPT 产品开展 周期性横向评测，输出能力对标分析。
人工评测体系建设
6) 设计端到端人工评测流程，结合问题点拆解与评分规则开展专家评测，对模型生成结果进行系统化质量评估。
7) 累计分析竞品对比和自身迭代，通过归因分析输出10+评测报告，定位模型在结构生成、排版稳定性等方面的能力短板，并及时反馈给算法团队。
自动化评测探索
8) 复用组内 Agent自动化评测框架，将人工评测标准结构化为 Rubric评分规则 并设计评测Prompt，接入 LLM-as-a-Judge 评测流程，实现对生成内容逻辑质量的自动化评分。
9) 通过对比 人工评分与LLM评分结果 开展人机一致性分析，持续优化Rubric判定逻辑与评测Prompt，提高自动评测结果的稳定性与可信度。
`,
